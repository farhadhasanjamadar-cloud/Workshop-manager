...
const showToast = (m) => {
  ...
  async function saveEnquiry(v) {
    // Keep the field names used by FormEnquiry consistently throughout the app.
    const isEdit = Boolean(modal.data?.id);
    const enquiry = isEdit
      ? { ...modal.data, ...v }
      : { ...v, id: uid(), createdAt: new Date().toISOString() };

    // Save locally immediately so the UI remains responsive.
    const list = data.enquiries || [];
    const next = isEdit
      ? list.map((r) => (r.id === enquiry.id ? enquiry : r))
      : [enquiry, ...list];
    persist("enquiries", next);

    // Keep the customer record locally as before.
    upsertCustomerRecord(enquiry.customerName, enquiry.phone, enquiry.location);
    ...
    try {
      ...
      try {
        await supabaseDbRequest("customers?on_conflict=phone", {
          ...
          body: JSON.stringify({
            name: enquiry.customerName || "Unnamed",
            phone: String(enquiry.phone || "").replace(/\s+/g, ""),
            address: enquiry.location || null,
          }),
        });
      } catch (customerError) {
        console.warn("Customer Supabase sync failed:", customerError);
        // Customer sync must not prevent the enquiry from being saved.
      }

      const payload = {
        enquiry_date: enquiry.date || todayISO(),
        ...
        enquiry_status: enquiry.status || "Warm",
        enquiry_outcome: enquiry.outcome || "Pending",
        next_followup_date: enquiry.nextFollowUp || null,
      ...
      // If this enquiry came from Supabase, update that row.
      // New enquiries are inserted and the returned database ID is stored locally.
      if (isEdit && enquiry.id && !/^[a-z0-9]+$/.test(enquiry.id)) {
        const updated = await supabaseDbRequest(
          `enquiries?id=eq.${encodeURIComponent(enquiry.id)}`,
          {
            method: "PATCH",
        ...
        remoteRecord = Array.isArray(updated) ? updated[0] : updated;
      } else {
        const inserted = await supabaseDbRequest("enquiries", {
          method: "POST",
          headers: { "Prefer": "return=representation" },
          body: JSON.stringify(payload),
        });
        remoteRecord = Array.isArray(inserted) ? inserted[0] : inserted;
      }

      // Replace the local temporary ID with Supabase's real ID for new records.
      if (remoteRecord?.id) {
        const synced = {
          ...enquiry,
          id: remoteRecord.id,
          createdAt: remoteRecord.created_at || enquiry.createdAt,
        };
        const current = data.enquiries || [];
        const replaced = current.map((r) => (r.id === enquiry.id ? synced : r));
        persist("enquiries", replaced);
      }

      showToast(isEdit ? "Enquiry updated & synced" : "Enquiry saved & synced");
    } catch (e) {
