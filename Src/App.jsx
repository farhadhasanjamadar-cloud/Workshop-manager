...
function Purchases({ items, onAdd, onEdit, onDelete, isManager = false }) {
...
function FormOpeningBalances({ data = [], customers = [], materials = [], onSave, onCancel }) {
  const [receivables, setReceivables] = useState([
    { customerPhone: "", amount: "", date: todayISO(), notes: "" }
  ]);
  const [payables, setPayables] = useState([
    { supplier: "", materialId: "", materialName: "", quantity: "", unit: "pcs", amount: "", amountPaid: "0", date: todayISO(), notes: "", linkStock: false }
  ]);
  ...
  const selectedCustomer = (row) =>
    customers.find(c => String(c.phone || "").replace(/\s+/g, "") === String(row.customerPhone || "").replace(/\s+/g, ""));

  const validReceivables = receivables.filter(r => r.customerPhone && num(r.amount) > 0);
  const validPayables = payables.filter(r => String(r.supplier || "").trim() && String(r.materialName || "").trim() && num(r.amount) > 0);
  ...
  return (
    <>
      <div className="field-hint" style={{ marginBottom: 14 }}>
        Add each old outstanding balance separately. You can enter the customer/supplier name, amount, date and the material purchased.
        These are opening records, not today's transactions.
      </div>

      <SectionHead title="Amount to be received — Customer balances" />
        ...
        <div className="list-card" key={`recv-${index}`} style={{ marginBottom: 12 }}>
          <div className="two-col">
            <Field label="Customer" required>
              <select
                ...
                onChange={(e) => updateRow(setReceivables, index, "customerPhone", e.target.value)}
              >
                <option value="">Select customer</option>
                {customers.map(c => (
          ...
          <div className="two-col">
            ...
            <Field label="Customer name">
              <Input value={selectedCustomer(row)?.name || ""} readOnly />
            </Field>
          </div>
          <Field label="Notes">
            <TextArea value={row.notes} onChange={e => updateRow(setReceivables, index, "notes", e.target.value)} placeholder="Old balance / invoice details..." />
          </Field>
          {receivables.length > 1 && (
            <button type="button" className="btn ghost" onClick={() => setReceivables(rows => rows.filter((_, i) => i !== index))}>
              Remove customer
            </button>
          )}
        </div>
      ...
      <button
        type="button"
        className="btn ghost"
        onClick={() => setReceivables(rows => [...rows, { customerPhone: "", amount: "", date: todayISO(), notes: "" }])}
        style={{ marginBottom: 16 }}
      >
        + Add another customer
      </button>

      <SectionHead title="Amount to be paid — Supplier / Material balances" />
        ...
        <div className="list-card" key={`pay-${index}`} style={{ marginBottom: 12 }}>
          ...
          <Field label="Material procured" required>
            <select
              ...
              onChange={(e) => {
                const m = materials.find(x => String(x.id) === String(e.target.value));
                setPayables(rows => rows.map((r, i) => i === index ? {
                  ...r,
            ...
            >
              ...
              {materials.map(m => (
                <option key={m.id} value={m.id}>{m.name} · {m.unit || "pcs"}</option>
              ))}
            </select>
          </Field>
          ...
          <div className="two-col">
            <Field label="Total amount owed" required>
          ...
          <Field label="Notes">
            <TextArea value={row.notes} onChange={e => updateRow(setPayables, index, "notes", e.target.value)} placeholder="Old supplier bill / material details..." />
          </Field>

          <label className="checkline">
            <input
              type="checkbox"
              checked={!!row.linkStock}
              onChange={(e) => updateRow(setPayables, index, "linkStock", e.target.checked)}
            />
            Add this historical quantity to current inventory stock
          </label>

          {payables.length > 1 && (
            <button type="button" className="btn ghost" onClick={() => setPayables(rows => rows.filter((_, i) => i !== index))}>
      ...
      <button
        type="button"
        className="btn ghost"
        onClick={() => setPayables(rows => [...rows, { supplier: "", materialId: "", materialName: "", quantity: "", unit: "pcs", amount: "", amountPaid: "0", date: todayISO(), notes: "", linkStock: false }])}
        style={{ marginBottom: 16 }}
      >
        + Add another supplier/material
      </button>
    ...
    try {
      ...
      for (const r of (v.receivables || [])) {
        const customer = (data.customers || []).find(c =>
          String(c.phone || "").replace(/\s+/g, "") === String(r.customerPhone || "").replace(/\s+/g, "")
        );
        if (!customer) throw new Error("Select a valid customer for every receivable balance.");
        ...
        const record = {
          ...
          notes: r.notes || "Opening customer balance"
        };
      ...
      for (const r of (v.payables || [])) {
        const material = (data.materials || []).find(m => String(m.id) === String(r.materialId));
        if (!material) throw new Error("Select a material for every supplier opening bill.");

        const openingNotes = [
          "Opening Balance Purchase",
          r.notes || "",
          `Historical material: ${material.name}`
        ].filter(Boolean).join(" | ");

        const record = {
          date: r.date || todayISO(),
          item: r.materialName || material.name,
          materialId: material.id,
          quantity: num(r.quantity),
          unit: r.unit || material.unit || "pcs",
          totalBill: num(r.amount),
          amountPaid: Math.min(Math.max(num(r.amountPaid), 0), num(r.amount)),
          party: String(r.supplier || "").trim(),
          notes: openingNotes,
          linkStock: !!r.linkStock
        };

        if (!record.party) throw new Error("Enter a supplier name for every supplier opening bill.");
        ...
        // Historical purchase can optionally establish opening stock.
        if (record.linkStock && num(record.quantity) > 0) {
          const materialNow = (data.materials || []).find(m => String(m.id) === String(material.id));
          if (materialNow) {
            const updatedMaterial = {
              ...materialNow,
              stock: num(materialNow.stock) + num(record.quantity),
              price: record.quantity > 0 && record.totalBill > 0
                ? Math.round(record.totalBill / record.quantity)
                : materialNow.price
            };
            const savedMaterial = await cloudUpsert("materials", updatedMaterial, materialNow);
    ...
    <div className="app">
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {page === "purchases" && !hasAccessLevel(sessionUser, "Manager") && <Purchases items={data.purchases} onEdit={(r) => openEdit("purchases", r)} onDelete={(id, l) => askDelete("purchases", id, l)} isManager={false} />}
      ...
      <Drawer open={!!modal && (!sessionUser || canAccessPage(sessionUser, modal.type === "openingBalances" ? "accounts" : modal.type))} onClose={closeModal} title={modalTitle(modal, page)}>
...
function modalTitle(modal, page) {
  ...
  const map = { enquiries: "Enquiry", quotations: "Quotation", jobs: "Job", customers: "Customer", inventory: "Material", purchases: "Purchase", finance: "Entry", openingBalances: "Opening Balances", staff: "Staff", notes: "Note" };
...
@media(max-width:859px){
  ...
  .drawer{width:min(92vw,380px);max-width:100%;}
  .drawer-body{padding:14px;gap:12px;padding-bottom:max(14px,env(safe-area-inset-bottom));}
  .form-actions{position:sticky;bottom:0;background:var(--surface);padding:10px 0 max(10px,env(safe-area-inset-bottom));z-index:2;}
