  ...
  async function loadRemoteEnquiries() {
    ...
    try {
      ...
      const remoteEnquiries = await supabaseDbRequest(
        "enquiries?select=*,customer:customers(name,phone,address)&order=enquiry_date.desc"
      );

      if (!Array.isArray(remoteEnquiries)) return;
      ...
      const mappedEnquiries = remoteEnquiries.map((r) => ({
        ...
        location: r.location || r.customer?.address || "",
      ...
      try { localStorage.setItem("enquiries", JSON.stringify(mappedEnquiries)); } catch (e) { /* cache is optional */ }
    } catch (e) {
