                ...
                <div className="record-actions">
                  ...
                  {canAdvance && onIssueMaterial && materials.length > 0 && (
                    <button className="chip-btn" onClick={(ev) => {
                      ev.stopPropagation();
                      const materialName = window.prompt("Material name to issue:");
                      if (!materialName) return;
                      const material = materials.find(m => String(m.name || "").toLowerCase() === materialName.trim().toLowerCase());
                      if (!material) { alert("Material not found in Inventory. Please add it first."); return; }
                      const quantity = window.prompt(`Quantity to issue for ${material.name}:`, "1");
                      if (quantity === null) return;
                      onIssueMaterial(j, material, quantity);
                    }}>
                      Issue Material
                    </button>
                  )}
