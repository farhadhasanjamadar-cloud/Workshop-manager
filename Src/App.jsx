...
const showToast = (m) => {
  ...
  async function remove(key, id) {
    try {
      const deleting = (data[key] || []).find(r => String(r.id) === String(id));

      // Money In is a ledger entry that can also update an order's advance.
      // When a Money In entry is deleted, reverse that linked payment first so
      // balances, receivables and dashboard alerts remain consistent.
      if (key === "finance" && deleting?.type === "in" && num(deleting.amount) > 0) {
        let target = null;
        if (deleting.productionJobId) {
          target = (data.jobs || []).find(j => String(j.id) === String(deleting.productionJobId)) || null;
        }
        if (!target && deleting.customerId) {
          const matches = (data.jobs || []).filter(j => String(j.customerId || "") === String(deleting.customerId));
          target = matches.length === 1 ? matches[0] : matches.sort((a,b) => String(b.date||"").localeCompare(String(a.date||"")))[0] || null;
        }
        if (!target && deleting.customerPhone) {
          const phone = String(deleting.customerPhone).replace(/\s+/g, "");
          const matches = (data.jobs || []).filter(j => String(j.phone || "").replace(/\s+/g, "") === phone);
          target = matches.length === 1 ? matches[0] : matches.sort((a,b) => String(b.date||"").localeCompare(String(a.date||"")))[0] || null;
        }
        if (target && num(target.advanceReceived) > 0) {
          const reversedAdvance = Math.max(num(target.advanceReceived) - num(deleting.amount), 0);
          const updatedJob = { ...target, advanceReceived: reversedAdvance };
          ...
          setData(d => ({ ...d, jobs: (d.jobs || []).map(j => String(j.id) === String(mappedJob.id) ? mappedJob : j) }));
        }
      }
