...
function AppInner() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setData(obj);
            // Enquiries are loaded again after sign-in as well; the first page load can
      // happen before a brand-new device has a Supabase session.
...
const showToast = (m) => {
  ...
  async function loadRemoteData() {
    ...
    try {
      const paths = [
        "customers?select=*&order=created_at.desc", "enquiries?select=*&order=enquiry_date.desc,created_at.desc",
        "quotations?select=*&order=quotation_date.desc,created_at.desc", "production_jobs?select=*&order=job_date.desc,created_at.desc",
        "materials?select=*&order=name.asc", "purchases?select=*&order=purchase_date.desc,created_at.desc",
        "finance_entries?select=*&order=entry_date.desc,created_at.desc", "staff?select=*&order=created_at.desc", "notes?select=*&order=updated_at.desc"
      ];
      const [customers, enquiries, quotations, jobs, materials, purchases, finance, staff, notes] = await Promise.all(paths.map((x)=>supabaseDbRequest(x)));
      const cb = new Map((customers||[]).map(c=>[String(c.id),c]));
      const mapped = {
        customers:(customers||[]).map(r=>mapCloudRecord("customers",r,cb)),
        enquiries:(enquiries||[]).map(r=>({ id:r.id,date:r.enquiry_date||"",customerName:cb.get(String(r.customer_id))?.name||"",phone:cb.get(String(r.customer_id))?.phone||"",location:r.location||cb.get(String(r.customer_id))?.address||"",callDetails:r.call_details||"",customerType:r.customer_type||"",requirement:r.requirement||"",customerBudget:r.customer_budget??"",amountQuoted:r.amount_quoted??"",status:r.enquiry_status||"Warm",outcome:r.enquiry_outcome||"Pending",nextFollowUp:r.next_followup_date||"",nextAction:r.next_action||"",remarks:r.remarks||""})),
        quotations:(quotations||[]).map(r=>mapCloudRecord("quotations",r,cb)), jobs:(jobs||[]).map(r=>mapCloudRecord("jobs",r,cb)), materials:(materials||[]).map(r=>mapCloudRecord("materials",r,cb)), purchases:(purchases||[]).map(r=>mapCloudRecord("purchases",r,cb)), finance:(finance||[]).map(r=>mapCloudRecord("finance",r,cb)), staff:(staff||[]).map(r=>mapCloudRecord("staff",r,cb)), notes:(notes||[]).map(r=>mapCloudRecord("notes",r,cb))
      };
      setData(mapped);
      Object.entries(mapped).forEach(([k,v])=>{ try{localStorage.setItem(k,JSON.stringify(v));}catch(e){} });
      return true;
    } catch(e) { console.error("Supabase data load failed:",e); return false; }
  }
  ...
  async function doDelete() { if (confirm) { const c=confirm; setConfirm(null); await remove(c.key,c.id); } }
  ...
  async function advanceStage(job,stage) { await saveJob({...job,stage,...stampJobStage(job,{stage})}); }
