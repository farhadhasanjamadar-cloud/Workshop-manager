...
const showToast = (m) => {
  ...
  async function cloudUpsert(key, record, existingRecord = null) {
    ...
    const current = existingRecord || record;
    let payload = {};
    ...
    // Resolve customer foreign keys from phone for quotations/jobs/finance.
    if (["quotations", "jobs", "finance"].includes(key) && record.phone) {
      const rows = await supabaseDbRequest(`customers?phone=eq.${encodeURIComponent(String(record.phone).replace(/\s+/g, ""))}&select=id`);
  ...
  async function saveQuotation(v) {
    ...
    try {
      if (!supabaseConfigured) { upsert("quotations", clean); closeModal(); return; }
      const saved = await cloudUpsert("quotations", clean, modal.data);
  ...
  async function convertQuoteToJob(q) {
    await saveJob({date:todayISO(),customerName:q.customerName,phone:q.phone,address:q.location,requirement:q.requirement,measurements:q.size,fabricType:q.fabric,foamType:"",frameSpec:"",quantity:1,priority:"Normal",dueDate:"",assignedStaff:"",quotedAmount:q.price,advanceReceived:"",stage:"Confirmed",deliveredAt:"",completedAt:"",specialInstructions:q.notes,quotationId:q.id});
    try { await cloudUpsert("quotations",{...q,status:"Converted"},q); persist("quotations",data.quotations.map(x=>x.id===q.id?{...x,status:"Converted"}:x)); showToast("Converted to job & synced"); } catch(e){console.error(e);} }
