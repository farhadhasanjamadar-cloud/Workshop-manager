...
const showToast = (m) => {
  ...
  async function cloudUpsert(key, record, existingRecord = null) {
    ...
    if (["quotations", "jobs", "finance"].includes(key) && record.phone) {
      const customer = await ensureCloudCustomer(record);
  ...
  function mapCloudRecord(key, r, customersById = new Map()) {
    ...
    if (key === "finance") return { id:r.id, date:r.entry_date||"", type:r.entry_type||"out", category:r.category||"", amount:r.amount??0, party:r.party_name||"", paymentMethod:r.payment_method||"", notes:r.notes||"" };
  ...
  async function saveFinance(v) { try { const saved=await cloudUpsert("finance",modal.data?{...modal.data,...v}:v,modal.data); const mapped=mapCloudRecord("finance",saved); persist("finance",modal.data?data.finance.map(x=>x.id===mapped.id?mapped:x):[mapped,...data.finance]); closeModal();showToast("Finance saved & synced"); } catch(e){console.error(e);setToast("NOT SYNCED: "+(e?.message||"Finance save failed"));setTimeout(()=>setToast(""),15000);} }
