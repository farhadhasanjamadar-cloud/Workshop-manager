...
const ACCESS_PERMISSIONS = {
  ...
  Accountant: ["today", "purchases", "finance", "accounts"],
  Manager: ["today", "jobs", "inventory", "purchases"],
  "Team worker": ["today", "jobs"],
  Sales: ["today", "enquiries"],
};
...
function normalizeAccessLevel(level, role) {
  ...
  // Legacy value used by the old Owner record.
  if (value === "Staff" && /^owner$/i.test(roleValue)) return "Owner";
  if (value === "Staff") return "Team worker";
  if (value === "View only") return "Sales";

  if (/^owner$/i.test(roleValue)) return "Owner";
  if (/^accountant$/i.test(roleValue)) return "Accountant";
  if (/^manager$/i.test(roleValue)) return "Manager";
  if (/^sales$/i.test(roleValue)) return "Sales";
  if (/^team\s*worker$/i.test(roleValue) || /^worker$/i.test(roleValue)) return "Team worker";

  return "Sales";
}

function allowedPagesForUser(user) {
  const level = normalizeAccessLevel(user?.accessLevel, user?.role);
  return ACCESS_PERMISSIONS[level] || ["today"];
}
...
function FormEnquiry({ data, customers, onSave, onCancel }) {
  ...
  function handleSave() {
    ...
    onSave({
      ...
      requirement,
      ...(isManager ? {
        quotedAmount: data?.quotedAmount ?? v.quotedAmount,
        advanceReceived: data?.advanceReceived ?? v.advanceReceived
      } : {})
    });
  }
  return (
...
function FormMaterial({ data, onSave, onCancel, isManager = false }) {
  ...
  return (
    <>
      ...
      <FormActions onCancel={onCancel} onSave={() => v.name && onSave(isManager ? { ...v, price: data?.price ?? v.price } : v)} disabled={!v.name} />
    </>
  );
}
...
function FormPurchase({ data, materials, onSave, onCancel, isManager = false }) {
  ...
  return (
    <>
      ...
      <div className="two-col">
        ...
        {!isManager && (
          <Field label="Total bill amount">
            <Input type="number" value={v.totalBill} onChange={set("totalBill")} />
          </Field>
        )}
        {isManager && (
          <Field label="Purchase price" hint={data?.id ? "Hidden for saved purchases." : "Enter the price now. It will be hidden after you save."}>
            <Input type="number" value={data?.id ? "" : v.totalBill} onChange={set("totalBill")} readOnly={!!data?.id} placeholder={data?.id ? "Hidden" : ""} />
          </Field>
        )}
      </div>
      {!isManager && <Field label="Amount paid"><Input type="number" value={v.amountPaid} onChange={set("amountPaid")} /></Field>}
      ...
      <FormActions
        onCancel={onCancel}
        onSave={() => v.item && v.party && v.quantity && onSave(
          isManager && data?.id
            ? { ...v, totalBill: data?.totalBill ?? v.totalBill, amountPaid: data?.amountPaid ?? v.amountPaid }
            : v
        )}
        disabled={!v.item || !v.party || !v.quantity}
      />
    </>
  );
}
...
function FormStaff({ data, onSave, onCancel }) {
  ...
  return (
    <>
      ...
      <Field label="App access level"><Select value={v.accessLevel || "Team worker"} onChange={set("accessLevel")} options={ACCESS_LEVELS} /></Field>
...
function TodayDashboard({ jobs, enquiries, quotations, finance, materials, isManager = false }) {
  ...
  const dueSoonJobs = jobs.filter((j) => {
    if (j.stage === "Delivered" || !j.dueDate) return false;
  ...
  const alerts = isManager
    ? [
        ...
        ...dueSoonJobs.map((j) => ({
          ...
          text: `${j.customerName}'s order is ${j.dueDate < today ? "overdue" : "due soon"} (${fmtDate(j.dueDate)})`,
          page: "jobs"
        })),
      ]
    : [
          ...
          <div className="card-list">
            {sorted.map((p) => (
              <div className="record-card" key={p.id} onClick={() => onEdit(p)}>
                ...
                {!isManager && {!isManager ? <div className="record-sub">Bill {fmtINR(p.totalBill)} · Paid {fmtINR(p.amountPaid)}</div> : <div className="record-sub">Purchase recorded</div>}}
                {isManager && <div className="record-sub">Purchase recorded</div>}
          ...
          <div className="card-list">
            {suppliers.map((s) => (
              <div className={`record-card ${s.balance > 0 ? "record-card-alert" : ""}`} key={s.name}>
                ...
                {!isManager && {!isManager ? <div className="record-sub">Purchased {fmtINR(s.purchased)} · Paid {fmtINR(s.paid)}</div> : <div className="record-sub">{s.txns.length} purchase transaction{s.txns.length > 1 ? "s" : ""}</div>}}
                {isManager && <div className="record-sub">{s.txns.length} purchase transaction{s.txns.length > 1 ? "s" : ""}</div>}
                ...
                <div className="supplier-txns">
                  {s.txns.slice(0, 5).map((t) => (
                    <div className="supplier-txn-row" key={t.id}>
                      ...
                      {!isManager && {!isManager && <span>{fmtINR(t.totalBill)}</span>}}
                    </div>
                  ))}
                </div>
              </div>
...
const showToast = (m) => {
  ...
  async function saveStaffRec(v) {
    try {
      ...
      if (needsLoginSetup) {
        ...
        // The Edge Function already links the Auth user to this staff row.
        // Do not write auth_user_id a second time: the unique constraint
        // staff_auth_user_id_key rejects that duplicate link.
        finalSaved = {
          ...saved,
          email,
          auth_user_id: json?.auth_user_id || saved.auth_user_id || null
        };
  ...
  useEffect(() => {
    if (sessionUser && !canAccessPage(sessionUser, page)) {
      setPage("today");
    }
  }, [sessionUser, page]);
    ...
    <div className="app">
      ...
      <nav className={`side-nav ${drawerOpen ? "open" : ""}`}>
        ...
        <div className="side-links">
          {NAV.filter((n) => canAccessPage(sessionUser, n.key)).map((n) => {
            const Icon = n.icon;
            return (
