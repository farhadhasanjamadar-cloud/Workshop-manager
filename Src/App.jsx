  ...
  const addAction = canAddOnPage ? (
    <button className="btn primary add-btn" onClick={() => openAdd(page)}>
      <Plus size={16} /> {PAGE_ADD_LABEL[page]}
              ...
              <button className="btn secondary" onClick={(ev) => {
                ev.stopPropagation();
                const materialName = window.prompt("Material name to issue:");
                if (!materialName) return;
                const material = (data.materials || []).find(m =>
                  String(m.name || "").toLowerCase() === materialName.trim().toLowerCase()
                );
                if (!material) {
                  alert("Material not found in Inventory. Please add it first.");
                  return;
                }
                const quantity = window.prompt(`Quantity to issue for ${material.name}:`, "1");
                if (quantity === null) return;
                issueMaterialToProduction(job, material, quantity);
