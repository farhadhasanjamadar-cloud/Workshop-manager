...
const ACCESS_PERMISSIONS = {
  ...
  Manager: ["jobs", "inventory", "purchases"],
  "Team worker": ["jobs"],
  Sales: ["enquiries"],
};
...
function normalizeAccessLevel(level, role) {
  ...
  if (ACCESS_PERMISSIONS[value]) return value;
  if (value === "Staff") return "Team worker";
  if (value === "View only") return "Sales";
  return "Sales";
}

function allowedPagesForUser(user) {
  const level = normalizeAccessLevel(user?.accessLevel);
  return ACCESS_PERMISSIONS[level] || [];
}
...
function FormStaff({ data, onSave, onCancel }) {
  ...
  return (
    <>
      ...
      <Field label="App access level" required hint="Controls which menus this staff member can see">
        <Select value={v.accessLevel || "Team worker"} onChange={set("accessLevel")} options={ACCESS_LEVELS} />
      </Field>
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        <div className="content">
          {!pageAllowed ? (
            ...
            <>
          {page === "today" && <TodayDashboard jobs={data.jobs} enquiries={data.enquiries} quotations={data.quotations} finance={data.finance} materials={data.materials} isManager={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Manager"} />}
          ...
          {page === "jobs" && <Jobs items={data.jobs} onEdit={(r) => openEdit("jobs", r)} onDelete={(id, l) => askDelete("jobs", id, l)} onAdvance={advanceStage} isManager={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Manager"} isViewOnly={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Team worker"} />}
          ...
          {page === "inventory" && <Inventory items={data.materials} onEdit={(r) => openEdit("inventory", r)} onDelete={(id, l) => askDelete("materials", id, l)} isManager={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Manager"} />}
          {page === "purchases" && <Purchases items={data.purchases} onEdit={(r) => openEdit("purchases", r)} onDelete={(id, l) => askDelete("purchases", id, l)} isManager={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Manager"} />}
      ...
      <Drawer open={!!modal && (!sessionUser || canAccessPage(sessionUser, modal.type))} onClose={closeModal} title={modalTitle(modal, page)}>
        ...
        {modal && modal.type === "jobs" && <FormJob data={modal.data} staff={data.staff} customers={data.customers} onSave={saveJob} onCancel={closeModal} isManager={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Manager"} isViewOnly={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Team worker"} />}
        {modal && modal.type === "customers" && <FormCustomer data={modal.data} onSave={saveCustomer} onCancel={closeModal} />}
        {modal && modal.type === "inventory" && <FormMaterial data={modal.data} onSave={saveMaterial} onCancel={closeModal} isManager={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Manager"} />}
        {modal && modal.type === "purchases" && <FormPurchase data={modal.data} materials={data.materials} onSave={savePurchase} onCancel={closeModal} isManager={normalizeAccessLevel(sessionUser?.accessLevel, sessionUser?.role) === "Manager"} />}
