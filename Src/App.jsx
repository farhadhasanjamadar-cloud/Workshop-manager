...
const FIN_CATEGORIES_IN = ["Sale / Payment Received", "Advance Received", "Other Income", "Opening Balance Receivable"];
...
function FormFinance({ data, customers = [], jobs = [], purchases = [], onSave, onCancel }) {
  const [v, set, setV] = useForm(data || { type: "out", date: todayISO(), category: "", amount: "", party: "", customerPhone: "", jobId: "", notes: "" });
  const cats = v.type === "in" ? FIN_CATEGORIES_IN : FIN_CATEGORIES_OUT;
  const activeJobs = (jobs || []).filter(j => j.stage !== "Delivered");
  const selectedCustomer = (customers || []).find(c => String(c.phone || "").replace(/\s+/g, "") === String(v.customerPhone || "").replace(/\s+/g, ""));
  const customerJobs = v.customerPhone
    ? activeJobs.filter(j => String(j.phone || "").replace(/\s+/g, "") === String(v.customerPhone).replace(/\s+/g, ""))
    : [];
  ...
  function chooseCustomer(e) {
    ...
    const c = (customers || []).find(x => String(x.phone || "").replace(/\s+/g, "") === phone);
    const firstJob = activeJobs.find(j => String(j.phone || "").replace(/\s+/g, "") === phone);
    setV(s => ({ ...s, customerPhone: phone, party: c?.name || s.party, jobId: firstJob?.id || "" }));
  }

  function selectSupplier(name) {
    const bills = pendingSupplierBills.filter(p => String(p.party || "").trim() === String(name || "").trim());
    const first = bills[0];
    setV(s => ({
      ...s,
      supplierName: name,
      supplierPurchaseId: first?.id || "",
      party: name || "",
      amount: first ? first.due : "",
  ...
  function enableSupplierPayment(enabled) {
    setV(s => ({
      ...
      supplierPurchaseId: enabled ? (s.supplierPurchaseId || supplierBills[0]?.id || "") : "",
      supplierName: enabled ? (s.supplierName || supplierBills[0]?.party || "") : "",
      party: enabled ? (s.party || supplierBills[0]?.party || "") : s.party,
      amount: enabled ? (s.amount || selectedSupplierBill?.due || supplierBills[0]?.due || "") : s.amount,
  ...
  return (
    <>
      <div className="toggle-row">
        <button type="button" className={`toggle ${v.type === "in" ? "toggle-in active" : ""}`} onClick={() => setV(s => ({ ...s, type: "in", supplierPayment: false, category: s.category || "Advance Received" }))}>Money In</button>
        <button type="button" className={`toggle ${v.type === "out" ? "toggle-out active" : ""}`} onClick={() => setV(s => ({ ...s, type: "out", category: s.category || "Material Purchase" }))}>Money Out</button>
      </div>
      ...
      {isSupplierPayment && (
        <>
          <Field label="Pending Supplier" required hint="Only suppliers with an outstanding bill are shown">
            <select className="ctrl" value={selectedSupplier} onChange={(e) => selectSupplier(e.target.value)}>
              ...
              {supplierNames.map(name => {
                const due = pendingSupplierBills.filter(p => String(p.party || "").trim() === name).reduce((sum, p) => sum + p.due, 0);
          ...
          <Field label="Payment Amount" required hint={selectedSupplierBill ? `Maximum for this bill: ${fmtINR(selectedSupplierBill.due)}` : "Enter the amount paid"}>
            <Input type="number" value={v.amount} onChange={set("amount")} max={selectedSupplierBill?.due || undefined} />
          </Field>
        </>
      )}

      {!isSupplierPayment && <Field label="Amount" required><Input type="number" value={v.amount} onChange={set("amount")} /></Field>}
        ...
        <>
          <Field label="Customer" required hint="Select the customer whose order this payment is for">
            <select className="ctrl" value={v.customerPhone} onChange={chooseCustomer}>
              ...
              {(customers || []).map(c => <option key={c.id} value={String(c.phone || "").replace(/\s+/g, "")}>{c.name} — {c.phone}</option>)}
            </select>
          </Field>
          {v.customerPhone && (
            <Field label="Order" hint={customerJobs.length > 1 ? "Choose the order receiving this payment" : "This order will be updated automatically"}>
              <select className="ctrl" value={v.jobId} onChange={set("jobId")}>
                ...
                {customerJobs.map(j => <option key={j.id} value={j.id}>{j.requirement || "Order"} — {fmtINR(j.quotedAmount)} — {j.stage}</option>)}
              </select>
            </Field>
          )}
          <div className="balance-line">
            ...
            })()}</> : "Select a customer to link this payment to an order."}
          </div>
        </>
      ) : (
        !isSupplierPayment && <Field label="Party" hint="Who paid / was paid"><Input value={v.party} onChange={set("party")} /></Field>
      )}

      {isSupplierPayment && <Field label="Payment Method"><Input value={v.paymentMethod || ""} onChange={set("paymentMethod")} placeholder="Cash / UPI / Bank / Cheque" /></Field>}

      <Field label="Notes"><TextArea value={v.notes} onChange={set("notes")} /></Field>
      <FormActions
        onCancel={onCancel}
        onSave={() => v.amount && onSave(v)}
        disabled={!v.amount || (v.type === "in" && !v.customerPhone) || (isSupplierPayment && !v.supplierPurchaseId)}
      />
    </>
  );
}
...
function Finance({ items, onAdd, onEdit, onDelete }) {
  ...
  const moneyIn = filtered
    .filter((f) => f.type === "in" && f.category !== "Opening Balance Receivable")
    .reduce((s, f) => s + num(f.amount), 0);
  const moneyOut = filtered
    .filter((f) => f.type === "out" && f.category !== "Opening Balance Payable")
    .reduce((s, f) => s + num(f.amount), 0);
  const sorted = [...filtered].sort((a, b) => (b.date || "").localeCompare(a.date || ""));
  return (
    <>
...
const showToast = (m) => {
  ...
  return (
    <div className="app">
      ...
      <Drawer open={!!modal && (!sessionUser || canAccessPage(sessionUser, ["openingBalances","customerHistory"].includes(modal.type) ? (modal.type === "customerHistory" ? "customers" : "accounts") : (modal.type === "supplierHistory" ? "purchases" : modal.type)))} onClose={closeModal} title={modalTitle(modal, page)}>
        ...
        {modal && modal.type === "finance" && <FormFinance data={modal.data} customers={data.customers} jobs={data.jobs} purchases={data.purchases} onSave={saveFinance} onCancel={closeModal} />}
...
function modalTitle(modal, page) {
  ...
  const map = { enquiries: "Enquiry", quotations: "Quotation", jobs: "Job", customers: "Customer", inventory: "Material", purchases: "Purchase", finance: "Entry", openingBalances: "Opening Balances", customerHistory: "Customer Transaction History", supplierHistory: "Supplier Transaction History", staff: "Staff", notes: "Note" };
