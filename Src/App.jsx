...
function Finance({ items, onAdd, onEdit, onDelete, onOpeningCashAccount }) {
  ...
  const regular = f => !String(f.category || "").startsWith("Opening ");
...
function Accounts({ jobs, materials, purchases, finance = [], customers = [], onOpeningBalances }) {
  ...
  const openingReceivable = openingReceivables.reduce((s, f) => s + num(f.amount), 0);
          ...
          <div className="table-card">
            {openingReceivables.map((f) => (
              <div className="table-row" key={f.id}>
                ...
                <div className="row-right tone-warn"><strong>{fmtINR(f.amount)}</strong></div>
              </div>
            ))}
          </div>
        </>
