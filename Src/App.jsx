...
/* ---------------------------------------------------------------- */
/* Shared payment / balance helpers                                   */
/* ---------------------------------------------------------------- */
function paymentsForJob(job, finance = [], allJobs = []) {
  if (!job) return 0;
  const jobId = String(job.id || '');
  const customerId = String(job.customerId || '');
  const phone = String(job.phone || '').replace(/\s+/g, '');
  const exact = (finance || []).filter(f => f.type === 'in' && f.productionJobId && String(f.productionJobId) === jobId);
  if (exact.length) return exact.reduce((s, f) => s + num(f.amount), 0);
  const customerJobs = (allJobs || []).filter(j => {
    if (customerId && j.customerId) return String(j.customerId) === customerId;
    return phone && String(j.phone || '').replace(/\s+/g, '') === phone;
  });
  if (customerJobs.length === 1) {
    return (finance || []).filter(f => f.type === 'in' && !f.productionJobId &&
      ((customerId && f.customerId && String(f.customerId) === customerId) ||
       (phone && String(f.customerPhone || '').replace(/\s+/g, '') === phone))
    ).reduce((s, f) => s + num(f.amount), 0);
  }
  return 0;
}
function balanceForJob(job, finance = [], allJobs = []) {
  const quoted = Math.max(num(job?.quotedAmount), 0);
  const ledgerPaid = paymentsForJob(job, finance, allJobs);
  const paid = Math.max(num(job?.advanceReceived), ledgerPaid);
  return Math.max(quoted - paid, 0);
}
...
function FormFinance({ data, customers = [], jobs = [], finance = [], onSave, onCancel }) {
        ...
        <>
          ...
          <div className="balance-line">
            {selectedCustomer ? <>Payment from <strong>{selectedCustomer.name}</strong>{v.jobId && (() => {
              ...
              const bal = j ? balanceForJob(j, finance, jobs) : 0;
...
function TodayDashboard({ jobs, enquiries, quotations, finance, materials }) {
  ...
  const pendingCollection = jobs.reduce((s, j) => s + balanceForJob(j, finance, jobs), 0);
  ...
  const unpaidDelivered = jobs.filter((j) => j.stage === "Delivered" && balanceForJob(j, finance, jobs) > 0);
  ...
  const alerts = [
    ...
    ...unpaidDelivered.map((j) => ({ tone: "warn", text: `${j.customerName} was delivered but still owes ${fmtINR(balanceForJob(j, finance, jobs))}`, page: "accounts" })),
  ];
...
function BusinessDashboard({ jobs, finance, materials, staff, quotations }) {
  ...
  const outstanding = jobs.reduce((s, j) => s + balanceForJob(j, finance, jobs), 0);
...
/* ---------------------------------------------------------------- */
/* Page: Notes — owner only                                           */
/* ---------------------------------------------------------------- */

function Notes({ items = [], onEdit, onDelete }) {
  const [q, setQ] = useState("");
  const sorted = [...items]
    .filter((n) => ((n.title || "") + " " + (n.content || "") + " " + (n.category || "")).toLowerCase().includes(q.toLowerCase()))
  ...
  return (
    <>
      ...
      {sorted.length === 0 && <EmptyState icon={StickyNote} title="No notes yet" hint="Create a private owner note." />}
      <div className="card-list">
        {sorted.map((n) => (
          <div className="record-card note-card" key={n.id} onClick={() => onEdit(n)}>
            <div className="record-top">
              <span className="record-title">{n.title || "General"}</span>
              {n.pinned && <Badge tone="accent">Pinned</Badge>}
            </div>
            <div className="note-content">{n.content}</div>
            {n.updatedAt && <div className="record-foot">Updated {new Date(n.updatedAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</div>}
            <button className="del-btn" onClick={(ev) => { ev.stopPropagation(); onDelete(n.id, n.title || "Note"); }}>
              <Trash2 size={15} />
            </button>
          </div>
        ))}
...
function Customers({ items, jobs, finance = [], onEdit, onDelete }) {
  const [q, setQ] = useState("");
  const filtered = items.filter((c) => (c.name + c.phone + (c.address || "")).toLowerCase().includes(q.toLowerCase()));
  const sorted = [...filtered].sort((a, b) => a.name.localeCompare(b.name));
  const balanceOf = (c) => jobs.filter((j) => j.phone === c.phone).reduce((s, j) => s + balanceForJob(j, finance, jobs), 0);
...
function Accounts({ jobs, materials, purchases, finance = [] }) {
  const balanceOf = (j) => balanceForJob(j, finance, jobs);
  const totalReceivable = jobs.reduce((s, j) => s + balanceOf(j), 0);
  const stockValue = materials.reduce((s, m) => s + num(m.stock) * num(m.price), 0);
  const totalPurchases = purchases.reduce((s, p) => s + num(p.totalBill), 0);
  const balanceJobs = jobs.filter((j) => balanceOf(j) > 0);
  ...
  return (
    <>
      ...
      <div className="table-card">
        ...
        {balanceJobs.map((j) => (
          <div className="table-row" key={j.id}>
            <div><div className="row-title">{j.customerName}</div><div className="row-sub">{jobNumber(jobs, j.id)} · {j.stage}</div></div>
            <div className="row-right tone-warn"><strong>{fmtINR(balanceOf(j))}</strong></div>
          </div>
        ))}
      </div>
      ...
      <div className="table-card">
        ...
        {materials.map((m) => (
          <div className="table-row" key={m.id}>
            <div><div className="row-title">{m.name}</div><div className="row-sub">{m.stock} {m.unit || 'units'} · {fmtINR(m.price)}/unit</div></div>
      ...
      <div className="table-card">
        {sortedPurchases.length === 0 && <div className="list-row"><span className="row-sub">No purchases yet.</span></div>}
        {sortedPurchases.map((p) => (
          <div className="table-row" key={p.id}>
            <div><div className="row-title">{p.item}</div><div className="row-sub">{fmtDate(p.date)} · {p.party}</div></div>
...
function AppInner() {
  ...
  const isOwner = sessionUser?.role?.trim().toLowerCase() === "owner" || sessionUser?.accessLevel?.trim().toLowerCase() === "owner";

  useEffect(() => {
    if (page === "notes" && !isOwner) setPage("today");
  }, [page, isOwner]);
  ...
  useEffect(() => {
    (async () => {
      ...
      // Notes are private to the owner and are not exposed before sign-in.
      obj.notes = [];
      setData(obj);
  ...
  try {
    ...
    if (session && session.user) {
      ...
      if ((signedInUser.role || signedInUser.accessLevel || "").trim().toLowerCase() !== "owner") {
...
const showToast = (m) => {
  ...
  async function ensureCloudCustomer(record) {
    ...
    const rows = await supabaseDbRequest("customers?on_conflict=phone", {
      ...
      body: JSON.stringify({
        name: record.customerName || record.party || record.name || "Unnamed",
        phone,
  ...
  async function cloudUpsert(key, record, existingRecord = null) {
    ...
    if (key === "finance") payload = { entry_date: record.date || todayISO(), entry_type: record.type || "out", category: record.category || null, amount: num(record.amount), party_name: record.party || null, customer_id: record.customerId || null, production_job_id: record.jobId || record.productionJobId || null, payment_method: record.paymentMethod || null, notes: record.notes || null };
    if (key === "staff") payload = { name: record.name || "", phone: record.phone || null, role: record.role || null, department: record.department || null, joining_date: record.joiningDate || null, salary: num(record.salary), labour_per_unit: num(record.labourPerUnit), status: record.status || (record.active === false ? "inactive" : "active"), notes: record.notes || null, auth_user_id: record.auth_user_id || null };
  ...
  function mapCloudRecord(key, r, customersById = new Map()) {
    ...
    if (key === "finance") return { id:r.id, date:r.entry_date||"", type:r.entry_type||"out", category:r.category||"", amount:r.amount??0, party:r.party_name||c?.name||"", customerId:r.customer_id||null, productionJobId:r.production_job_id||null, customerPhone:c?.phone||"", paymentMethod:r.payment_method||"", notes:r.notes||"" };
    if (key === "staff") return { id:r.id, name:r.name||"", phone:r.phone||"", role:r.role||"", department:r.department||"", joiningDate:r.joining_date||"", salary:r.salary??"", labourPerUnit:r.labour_per_unit??"", status:r.status||"active", active:r.status!=="inactive", notes:r.notes||"", auth_user_id:r.auth_user_id||null, email:"" };
  ...
  function openAdd(type) {
    if (type === "notes" && !isOwner) return;
    setModal({ type, data: null });
  }
  function openEdit(type, record) {
    if (type === "notes" && !isOwner) return;
    setModal({ type, data: record });
  }
  function closeModal() { setModal(null); }
  function askDelete(key, id, label) {
    if (key === "notes" && !isOwner) return;
    setConfirm({ key, id, label });
  }
    ...
    try {
      ...
      if (_fromEnquiryId) {
        ...
        if (enquiry) {
          ...
          if (supabaseConfigured && /^[0-9a-f-]{36}$/i.test(String(enquiry.id))) {
            const rows = await supabaseDbRequest(`enquiries?id=eq.${encodeURIComponent(enquiry.id)}&select=*`, {
              method: "PATCH",
              headers: { "Prefer": "return=representation" },
              body: JSON.stringify({ enquiry_status: "Converted" })
            });
            const remote = Array.isArray(rows) ? rows[0] : rows;
            if (remote?.id) converted.status = remote.enquiry_status || "Converted";
          }
          persist("enquiries", data.enquiries.map(e => e.id === enquiry.id ? converted : e));
        }
      }
  ...
  async function saveStaffRec(v) { try { const saved=await cloudUpsert("staff",modal.data?{...modal.data,...v}:v,modal.data); const mapped=mapCloudRecord("staff",saved); persist("staff",modal.data?data.staff.map(x=>x.id===mapped.id?mapped:x):[mapped,...data.staff]); closeModal();showToast("Staff saved & synced"); } catch(e){console.error(e);setToast("NOT SYNCED: "+(e?.message||"Staff save failed"));setTimeout(()=>setToast(""),15000);} }
  async function saveNote(v) {
    if (!isOwner) return;
    try {
      const record = modal.data ? { ...modal.data, ...v, updatedAt: new Date().toISOString() } : { ...v, updatedAt: new Date().toISOString() };
      const saved = await cloudUpsert("notes", record, modal.data);
      const mapped = mapCloudRecord("notes", saved);
      persist("notes", modal.data ? data.notes.map(x => x.id === mapped.id ? mapped : x) : [mapped, ...data.notes]);
      closeModal();
      showToast("Note saved & synced");
    } catch(e) {
      console.error(e);
      setToast("NOT SYNCED: " + (e?.message || "Note save failed"));
      setTimeout(() => setToast(""), 15000);
    }
  }
  ...
  async function saveFinance(v) {
    try {
      const record = modal.data ? { ...modal.data, ...v } : { ...v };
      if (record.type === "in" && record.jobId) {
        const linkedJob = (data.jobs || []).find(j => String(j.id) === String(record.jobId));
        if (linkedJob) {
          record.customerId = linkedJob.customerId || record.customerId || null;
          record.productionJobId = linkedJob.id;
          record.customerPhone = linkedJob.phone || record.customerPhone || "";
          record.party = record.party || linkedJob.customerName || "";
        }
      }
      ...
      const nextFinance = modal.data
        ? data.finance.map(x => x.id === mapped.id ? mapped : x)
        : [mapped, ...data.finance];
      persist("finance", nextFinance);

      // Money In is the payment ledger. Every payment is tied to a specific
      // production order when possible, including delivered orders.
      if (record.type === "in" && num(record.amount) > 0) {
        ...
        if (!target && cleanPhone) {
          ...
          target = matches.length === 1
            ? matches[0]
            : matches.sort((a, b) => String(b.date || "").localeCompare(String(a.date || "")))[0];
        }

        if (target) {
          const exactPayments = nextFinance.filter(f =>
            f.type === "in" && f.productionJobId && String(f.productionJobId) === String(target.id)
          );
          const legacyPayments = nextFinance.filter(f =>
            f.type === "in" && !f.productionJobId &&
            String(f.customerPhone || "").replace(/\s+/g, "") === String(target.phone || "").replace(/\s+/g, "")
          );
          const ledgerPaid = exactPayments.length
            ? exactPayments.reduce((s, f) => s + num(f.amount), 0)
            : legacyPayments.reduce((s, f) => s + num(f.amount), 0);
          const newAdvance = Math.min(Math.max(num(target.quotedAmount), 0) || Infinity, Math.max(num(target.advanceReceived), ledgerPaid));
  ...
  if (gateRequired && !sessionUser) {
    return <LoginGate
      ...
      onSuccess={async (user) => {
        setSessionUser(user);
        await loadRemoteData();
        if ((user?.role || user?.accessLevel || "").trim().toLowerCase() !== "owner") {
          setData((current) => ({ ...current, notes: [] }));
        }
      }}
  ...
  const addAction = PAGE_ADD_LABEL[page] && (page !== "notes" || isOwner) ? (
    ...
    <div className="app">
      ...
      <nav className={`side-nav ${drawerOpen ? "open" : ""}`}>
        ...
        <div className="side-links">
          {NAV.filter((n) => n.key !== "notes" || isOwner).map((n) => {
            const Icon = n.icon;
            return (
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {page === "customers" && <Customers items={data.customers} jobs={data.jobs} finance={data.finance} onEdit={(r) => openEdit("customers", r)} onDelete={(id, l) => askDelete("customers", id, l)} />}
          ...
          {page === "accounts" && <Accounts jobs={data.jobs} materials={data.materials} purchases={data.purchases} finance={data.finance} />}
          ...
          {page === "notes" && isOwner && <Notes items={data.notes} onEdit={(r) => openEdit("notes", r)} onDelete={(id, l) => askDelete("notes", id, l)} />}
      ...
      <Drawer open={!!modal} onClose={closeModal} title={modalTitle(modal, page)}>
        ...
        {modal && modal.type === "finance" && <FormFinance data={modal.data} customers={data.customers} jobs={data.jobs} finance={data.finance} onSave={saveFinance} onCancel={closeModal} />}
        {modal && modal.type === "staff" && <FormStaff data={modal.data} onSave={saveStaffRec} onCancel={closeModal} />}
        {modal && modal.type === "notes" && isOwner && <FormNote data={modal.data} onSave={saveNote} onCancel={closeModal} />}
      </Drawer>
