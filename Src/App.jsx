...
const ACCESS_LEVELS = ["Owner", "Accountant", "Manager", "Team worker", "Sales"];
...
function unitLabel(value) {
  ...
  return u ? u.label : (value || "Pieces (pcs)");
}
...
const KEYS = ["enquiries", "quotations", "jobs", "materials", "purchases", "finance", "staff", "notes", "customers", "stock_movements"];
...
function FormEnquiry({ data, customers, onSave, onCancel }) {
  ...
  const reqValid = v.requirement !== "Others" || String(v.requirementOther || "").trim();
  const canSave = v.customerName.trim() && phoneValid && reqValid;
  function handleSave() {
    if (!canSave) return;
    const requirement = v.requirement === "Others" && String(v.requirementOther || "").trim() ? String(v.requirementOther).trim() : v.requirement;
...
function FormJob({ data, staff, customers, materialIssues = [], onSave, onCancel, isManager = false, isViewOnly = false, isOwner = false, jobCardNumber = "" }) {
  const readOnly = isManager || isViewOnly;
  ...
  return (
    <>
      ...
      {(isManager || isViewOnly) ? (
        <div className="job-private-banner">
          <div><strong>Job Card {v.jobNumber || jobCardNumber || "Assigned on save"}</strong></div>
          <div>Customer details are hidden for this access level.</div>
        </div>
      ) : (
        <>
          <Field label="Customer name" required hint="Start typing to find an existing customer">
      ...
      <SectionHead title="Materials Issued to This Job" />
      {(() => {
        const issued = (materialIssues || []).filter(x => String(x.jobId) === String(v.id));
        if (!issued.length) {
          return <div className="empty-inline">No materials issued to this job yet.</div>;
        }
        const totalMaterialCost = issued.reduce((sum, x) => sum + Number(x.materialCost || 0), 0);
        return (
          <div className="list-card" style={{ marginBottom: 12 }}>
            {issued.map((x, i) => (
              <div className="list-row" key={x.id || i}>
                <div>
                  <div className="row-title">{x.materialName}</div>
                  <div className="row-meta">{fmtDate(x.date)} · {x.issuedBy || "Production"}</div>
                </div>
                <div className="row-right">
                  <div className="row-meta"><strong>{x.quantity} {x.unit || ""}</strong></div>
                  {isOwner && <div className="row-sub">{fmtINR(Number(x.materialCost || 0))}</div>}
                </div>
              </div>
            ))}
            {isOwner && (
              <div className="list-row" style={{ borderTop: "1px solid var(--line)", marginTop: 4, paddingTop: 12 }}>
                <span className="row-title">Raw material cost used</span>
                <strong>{fmtINR(totalMaterialCost)}</strong>
              </div>
            )}
          </div>
        );
      ...
      {!readOnly && <FormActions onCancel={onCancel} onSave={handleSave} disabled={!canSave} />}
      </div>
    </>
  );
}
...
function FormMaterial({ data, onSave, onCancel, isManager = false }) {
  const [v, set] = useForm(data || { name: "", category: "", stock: 0, reorderLevel: 0, supplier: "", price: "", unit: "pcs" });
  return (
    <>
...
function FormFinance({ data, customers = [], jobs = [], onSave, onCancel }) {
  ...
  return (
    <>
      <div className="toggle-row">
        <button type="button" className={`toggle ${v.type === "in" ? "toggle-in active" : ""}`} onClick={() => setV(s => ({ ...s, type: "in", category: s.category || "Advance Received" }))}>Money In</button>
      ...
      <Field label="Notes"><TextArea value={v.notes} onChange={set("notes")} /></Field>
      <FormActions onCancel={onCancel} onSave={() => v.amount && onSave(v)} disabled={!v.amount || (v.type === "in" && !v.customerPhone)} />
    </>
  );
}
...
function jobNumber(jobs, id) {
  ...
  return `JC-${String(Math.max(idx + 1, 1)).padStart(3, "0")}`;
}

function nextJobNumber(jobs) {
  const max = (jobs || []).reduce((n, j) => {
    const m = String(j.jobNumber || "").match(/^JC-(\d+)$/i);
    return Math.max(n, m ? Number(m[1]) : 0);
  }, 0);
  return `JC-${String(Math.max(max, (jobs || []).length) + 1).padStart(3, "0")}`;
}

function TodayDashboard({ jobs, enquiries, quotations, finance, materials, sessionUser = null }) {
  ...
  const levels = normalizeAccessLevels(sessionUser?.accessLevel, sessionUser?.role);
  const isOwner = levels.includes("Owner");
  const isAccountant = levels.includes("Accountant");
  const isManager = levels.includes("Manager");
  const isTeamWorker = levels.includes("Team worker");
  const isSales = levels.includes("Sales");

  const lowStock = materials.filter((m) => num(m.stock) <= num(m.reorderLevel));

  // Enquiry alerts: follow-ups due today or overdue. Visible to Owner and Sales.
  const enquiryFollowUps = enquiries.filter((e) => {
    const status = String(e.status || "").toLowerCase();
    return e.nextFollowUp && e.nextFollowUp <= today && status !== "converted" && status !== "lost";
  });

  // Payment alerts: outstanding balances on active jobs. Visible to Owner and Accountant.
  ...
  // Production alerts: active jobs due today, overdue, or within the next 2 days.
  ...
  // Owner sees every alert. Other users see alerts relevant to their responsibility.
  const alerts = [];
  ...
  if (isOwner || isAccountant) {
    paymentDueJobs.forEach((j) => {
      ...
      alerts.push({
        tone: "warn",
        text: `Payment pending ${fmtINR(balance)} · ${jobNumber(jobs, j.id)}${isOwner && j.customerName ? ` · ${j.customerName}` : ""}`,
        page: "finance",
        category: "payment",
      });
    });
  ...
  if (isOwner || isManager || isTeamWorker) {
    lowStock.forEach((m) => {
      alerts.push({
        tone: "bad",
    ...
    dueJobs.forEach((j) => {
      // Team workers only receive production alerts for jobs assigned to them.
      // Managers receive all production alerts. Owner receives everything.
      if (isTeamWorker && !isManager && !isOwner &&
          String(j.assignedStaff || "").trim().toLowerCase() !== String(sessionUser?.name || "").trim().toLowerCase()) {
        return;
      }

      const card = jobNumber(jobs, j.id);
      const overdue = j.dueDate < today;
      const operationalText = `${card} · ${j.requirement || "Job"} is ${overdue ? "overdue" : `due ${fmtDate(j.dueDate)}`}`;
      alerts.push({
        tone: overdue ? "bad" : "warn",
        text: isOwner && j.customerName
          ? `${j.customerName}'s order (${card}) is ${overdue ? "overdue" : `due ${fmtDate(j.dueDate)}`}`
          : operationalText,
        page: "jobs",
        category: "production",
      });
  ...
  return (
    <>
      ...
      <div className="list-card alerts-card">
        ...
        {alerts.map((a, i) => (
          <div className="alert-row" key={`${a.category}-${i}`}>
...
function Jobs({ items, materials = [], materialIssues = [], onAdd, onEdit, onDelete, onAdvance, onIssueMaterial, isManager = false, isViewOnly = false, isOwner = false }) {
  const [q, setQ] = useState("");
  const [filter, setFilter] = useState("Active");
  const [issueJob, setIssueJob] = useState(null);
  const [issueMaterialId, setIssueMaterialId] = useState("");
  const [issueQuantity, setIssueQuantity] = useState("1");
  const privateView = isManager || isViewOnly;
  const canAdvance = isManager || !isViewOnly;
  const filtered = items
    .filter((j) => {
      const searchText = privateView
        ? `${jobNumber(items, j.id)} ${j.requirement || ""} ${j.assignedStaff || ""} ${j.stage || ""}`
        : `${j.customerName || ""} ${j.phone || ""} ${jobNumber(items, j.id)} ${j.requirement || ""}`;
  ...
  return (
    <>
      <SearchBar value={q} onChange={setQ} placeholder={privateView ? "Search job card, requirement or staff" : "Search jobs"} />
      ...
      <div className="card-list">
        {sorted.map((j) => {
          ...
          const jobIssues = (materialIssues || []).filter(x => String(x.jobId) === String(j.id));
          return (
            <div className="record-card" key={j.id} onClick={() => onEdit(j)}>
              <div className="record-top">
                <span className="record-title">{privateView ? jobNumber(items, j.id) : j.customerName}</span>
                <Badge tone={stageTone(j.stage)}>{j.stage}</Badge>
              </div>
              <div className="record-sub">
                {privateView ? `${j.requirement || "Job"} · Qty ${j.quantity || 1}` : `${jobNumber(items, j.id)} · ${j.requirement} · Qty ${j.quantity || 1}`}
              ...
              <div className="record-sub">
                ...
                {!privateView && bal > 0 && <span className="tone-warn"> · Balance {fmtINR(bal)}</span>}
              </div>
              {jobIssues.length > 0 && (
                <div className="record-sub" style={{ marginTop: 8 }}>
                  ...
                  {jobIssues.map((x, i) => (
                    <span key={x.id || i}>
                      {x.materialName} × {x.quantity} {x.unit || ""}
                      {i < jobIssues.length - 1 ? " · " : ""}
                    </span>
                  ))}
                  {isOwner && (
                    <div style={{ marginTop: 4 }}>
                      <strong>Raw material cost:</strong>{" "}
                      {fmtINR(jobIssues.reduce((sum, x) => sum + Number(x.materialCost || 0), 0))}
                    </div>
                  )}
                </div>
              )}
              {canAdvance && (
                <div className="record-actions">
                  ...
                  {canAdvance && onIssueMaterial && materials.length > 0 && (
              ...
              {!isManager && !isViewOnly && (
                <button className="del-btn" onClick={(ev) => { ev.stopPropagation(); onDelete(j.id, privateView ? jobNumber(items, j.id) : j.customerName); }}>
                  <Trash2 size={15} />
                </button>
              )}
            </div>
            ...
            <div className="form-grid">
              ...
              <Field label="Quantity to issue">
                <Input
                  ...
                  step="0.01"
                  value={issueQuantity}
                  onChange={(ev) => setIssueQuantity(ev.target.value)}
                />
              </Field>
            </div>
            ...
            <div className="modal-actions">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!material) {
                    setToast("Please select a material.");
                  ...
                  if (qty > Number(material.stock || 0)) {
                    setToast(`Not enough ${material.name} in stock. Available: ${Number(material.stock || 0)}`);
...
function Inventory({ items, onAdd, onEdit, onDelete, isManager = false, isOwner = false }) {
  const [q, setQ] = useState("");
          ...
          return (
            <div className={`table-row ${low ? "table-row-alert" : ""}`} key={m.id} onClick={() => onEdit(m)}>
              ...
              <div className="row-right">
                ...
                {isOwner && m.price ? <div className="row-sub">{fmtINR(m.price)} / {m.unit || "unit"}</div> : null}
              </div>
...
const showToast = (m) => {
  ...
  async function saveQuotation(v) {
    ...
    try {
      ...
      if (_fromEnquiryId) {
        const enquiry = (data.enquiries || []).find(e => String(e.id) === String(_fromEnquiryId));
        if (enquiry) {
          const enquiryPayload = {
            enquiry_status: "Converted"
          };
          await supabaseDbRequest(`enquiries?id=eq.${encodeURIComponent(_fromEnquiryId)}`, {
            method: "PATCH",
            headers: { "Prefer": "return=representation" },
            body: JSON.stringify(enquiryPayload)
          });
          persist("enquiries", (data.enquiries || []).map(e => e.id === enquiry.id ? { ...e, status: "Converted" } : e));
        }
      }
  ...
  async function saveJob(v) {
    if (hasAccessLevel(sessionUser, "Manager") || hasAccessLevel(sessionUser, "Team worker")) {
      setToast("Job cards are view-only for this access level.");
      setTimeout(() => setToast(""), 5000);
      return;
    }
    const prev=modal.data||{};
    const record={...v, jobNumber: v.jobNumber || prev.jobNumber || nextJobNumber(data.jobs), ...stampJobStage(prev,v)};
    try {
  ...
  async function saveFinance(v) {
    try {
      ...
      // Money In is the payment ledger. Automatically apply the payment to the
      // selected order so advance/balance and dashboard alerts stay correct.
      if (record.type === "in" && num(record.amount) > 0) {
        const cleanPhone = String(record.customerPhone || record.phone || "").replace(/\\s+/g, "");
        let target = record.jobId
          ? (data.jobs || []).find(j => String(j.id) === String(record.jobId))
          : null;
  ...
  async function convertQuoteToJob(q) {
    try {
      ...
      const jobRecord = {
        ...(existingJob || {}),
        jobNumber: existingJob?.jobNumber || nextJobNumber(data.jobs),
  ...
  async function issueMaterialToProduction(job, material, quantity) {
    ...
    if (!Number.isFinite(qty) || qty <= 0) {
      setToast("Enter a valid quantity to issue.");
      setTimeout(() => setToast(""), 5000);
      return;
    }
    ...
    const movement = {
      ...
      jobId: job.id,
      jobNumber: job.jobNumber || job.orderNo || job.id,
      quantity: qty,
    ...
    persist("materials", (data.materials || []).map(m =>
    ...
    try {
      if (supabaseConfigured && /^[0-9a-f-]{36}$/i.test(String(job.id))) {
        await supabaseDbRequest(
          `production_jobs?id=eq.${encodeURIComponent(job.id)}`,
          {
            method: "PATCH",
            headers: { "Prefer": "return=minimal" },
            body: JSON.stringify({ material_cost: Number(updatedJob.materialCost || 0) })
          }
        );
      }
      if (supabaseConfigured && /^[0-9a-f-]{36}$/i.test(String(material.id))) {
        const saved = await supabaseDbRequest(
          `materials?id=eq.${encodeURIComponent(material.id)}&select=*`,
          {
            method: "PATCH",
            headers: { "Prefer": "return=representation" },
  ...
  const canAddOnPage = pageAllowed && PAGE_ADD_LABEL[page] && !(page === "jobs" && hasAccessLevel(sessionUser, "Manager"));
  const addAction = canAddOnPage ? (
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        <div className="content">
          {!pageAllowed ? (
            ...
            <>
          {page === "today" && <TodayDashboard jobs={data.jobs} enquiries={data.enquiries} quotations={data.quotations} finance={data.finance} materials={data.materials} sessionUser={sessionUser} />}
          ...
          {page === "jobs" && <Jobs items={data.jobs} materials={data.materials} materialIssues={data.stock_movements || []} onEdit={(r) => openEdit("jobs", r)} onDelete={(id, l) => askDelete("jobs", id, l)} onAdvance={advanceStage} onIssueMaterial={issueMaterialToProduction} isManager={hasAccessLevel(sessionUser, "Manager")} isViewOnly={(hasAccessLevel(sessionUser, "Team worker") && !hasAccessLevel(sessionUser, "Manager"))} isOwner={hasAccessLevel(sessionUser, "Owner")} />}
          ...
          {page === "inventory" && <Inventory items={data.materials} onEdit={(r) => openEdit("inventory", r)} onDelete={(id, l) => askDelete("materials", id, l)} isManager={hasAccessLevel(sessionUser, "Manager")} isOwner={hasAccessLevel(sessionUser, "Owner")} />}
      ...
      <Drawer open={!!modal && (!sessionUser || canAccessPage(sessionUser, modal.type))} onClose={closeModal} title={modalTitle(modal, page)}>
        ...
        {modal && modal.type === "jobs" && <FormJob data={modal.data} staff={data.staff} customers={data.customers} materialIssues={data.stock_movements || []} onSave={saveJob} onCancel={closeModal} isManager={hasAccessLevel(sessionUser, "Manager")} isViewOnly={(hasAccessLevel(sessionUser, "Manager") || hasAccessLevel(sessionUser, "Team worker"))} isOwner={hasAccessLevel(sessionUser, "Owner")} jobCardNumber={jobNumber(data.jobs, modal.data?.id)} />}
        ...
        {modal && modal.type === "finance" && <FormFinance data={modal.data} customers={data.customers} jobs={data.jobs} onSave={saveFinance} onCancel={closeModal} />}
...
.job-private-banner{border:1px solid var(--border);background:var(--surface-2);border-radius:14px;padding:14px;margin-bottom:14px;color:var(--text-dim);}
.job-private-banner strong{display:block;color:var(--text);font-size:17px;margin-bottom:4px;}
