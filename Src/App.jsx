...
function FormOpeningBalances({ data = [], onSave, onCancel }) {
  ...
  const totalReceivable = num(v.receivable);
  const totalPayable = num(v.payable);

  return (
    <>
      <div className="field-hint" style={{ marginBottom: 12 }}>
        Enter the outstanding amounts that already exist today, before you start using the app.
        These are opening balances and are not treated as today's cash transactions.
      </div>

      <SectionHead title="Amount to be received" />
      <Field label="Opening customer receivable" hint="Total amount currently outstanding from all customers">
        <Input
          type="number"
          min="0"
          value={v.receivable}
      ...
      <Field label="Opening supplier payable" hint="Total amount currently outstanding to all suppliers">
        <Input
          type="number"
          min="0"
          value={v.payable}
      ...
      {(openingReceivable > 0 || openingPayable > 0) && (
        <div className="list-card" style={{ marginBottom: 12 }}>
          <Row label="Previously entered receivable" value={fmtINR(openingReceivable)} />
          <Row label="Previously entered payable" value={fmtINR(openingPayable)} />
        </div>
      )}

      <Field label="Notes" hint="Optional">
        <TextArea value={v.notes} onChange={set("notes")} placeholder="Opening balances as of today..." />
      </Field>

      <div className="field-hint" style={{ marginBottom: 12 }}>
        New opening balances entered here are added to the existing opening balance records.
      </div>

      <FormActions
        onCancel={onCancel}
        onSave={() => onSave(v)}
        disabled={totalReceivable <= 0 && totalPayable <= 0}
      />
    </>
  );
}
    ...
    <>
      ...
      <div className="section-head">
        <div>
          ...
          <div className="field-hint">Enter the outstanding amounts that existed before you started using the app.</div>
        </div>
        <button className="btn primary" onClick={onOpeningBalances}>
    ...
    try {
      ...
      if (num(v.receivable) > 0) {
        const record = {
          ...
          notes: note || "Opening customer receivable"
        };
      ...
      if (num(v.payable) > 0) {
        const record = {
          ...
          party: "Opening Balance",
          notes: note || "Opening supplier payable"
        };
        const saved = await cloudUpsert("finance", record, null);
        const mapped = mapCloudRecord("finance", saved, new Map((data.customers || []).map(c => [String(c.id), c])));
        created.push(mapped);
      ...
      if (!created.length) throw new Error("Enter at least one opening balance.");
