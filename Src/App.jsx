...
function FormOpeningBalances({ data = [], onSave, onCancel }) {
  const existingReceivable = (data || [])
    .filter((f) => f.category === "Opening Balance Receivable")
    .reduce((sum, f) => sum + num(f.amount), 0);

  const existingPayable = (data || [])
  ...
  const canSave = num(v.receivable) > 0 || num(v.payable) > 0;

  return (
    <>
      <div className="field-hint" style={{ marginBottom: 14 }}>
        Enter the outstanding balances that already existed before you started using the app.
        These will be added to Accounts but will not be counted as today's cash income or expense.
      </div>

      <SectionHead title="Amount to be received" />
      <Field label="Opening customer receivable" hint="Total amount currently outstanding from customers">
        <Input
          type="number"
          min="0"
          value={v.receivable}
      ...
      <Field label="Opening supplier payable" hint="Total amount currently outstanding to suppliers">
        <Input
          type="number"
          min="0"
          value={v.payable}
          onChange={set("payable")}
          placeholder="₹ 0"
        />
      </Field>

      {(existingReceivable > 0 || existingPayable > 0) && (
        <div className="list-card" style={{ marginBottom: 12 }}>
          <Row label="Already entered — receivable" value={fmtINR(existingReceivable)} />
          <Row label="Already entered — payable" value={fmtINR(existingPayable)} />
        </div>
      )}

      <Field label="Notes" hint="Optional">
        <TextArea value={v.notes} onChange={set("notes")} placeholder="Opening balances..." />
      </Field>
...
function Accounts({ jobs, materials, purchases, finance = [], onOpeningBalances }) {
  const openingReceivable = finance
    .filter((f) => f.category === "Opening Balance Receivable")
    .reduce((s, f) => s + num(f.amount), 0);

  const openingPayable = finance
    .filter((f) => f.category === "Opening Balance Payable")
    .reduce((s, f) => s + num(f.amount), 0);
  ...
  const totalPayable = supplierPayable + openingPayable;
  const stockValue = materials.reduce((s, m) => s + num(m.stock) * num(m.price), 0);
    ...
    <>
      ...
      <div className="section-head">
        <div>
          ...
          <div className="field-hint">Balances that existed before using the app.</div>
        </div>
...
const showToast = (m) => {
  ...
  async function cloudUpsert(key, record, existingRecord = null) {
    ...
    if (key === "purchases") payload = { purchase_date: record.date || todayISO(), purchase_item: record.item || "", quantity: num(record.quantity), unit: record.unit || "pcs", total_bill_amount: num(record.totalBill), amount_paid: num(record.amountPaid), party_name: record.party || null, notes: record.notes || null, add_to_inventory: !!record.linkStock };
  ...
  function mapCloudRecord(key, r, customersById = new Map()) {
    ...
    if (key === "purchases") return { id:r.id, date:r.purchase_date||"", item:r.purchase_item||"", quantity:r.quantity??0, unit:r.unit||"pcs", totalBill:r.total_bill_amount??0, amountPaid:r.amount_paid??0, party:r.party_name||"", notes:r.notes||"", linkStock:r.add_to_inventory!==false };
    if (key === "finance") return { id:r.id, date:r.entry_date||"", type:r.entry_type||"out", category:r.category||"", amount:r.amount??0, party:r.party_name||c?.name||"", customerPhone:c?.phone||"", paymentMethod:r.payment_method||"", notes:r.notes||"" };
  ...
  async function saveOpeningBalances(v) {
    try {
      ...
      if (num(v.receivable) > 0) {
        const record = {
          type: "in",
          date: todayISO(),
          category: "Opening Balance Receivable",
          amount: num(v.receivable),
          party: "Opening Balance",
          notes: String(v.notes || "").trim() || "Opening customer receivable"
        };
      ...
      if (num(v.payable) > 0) {
        const record = {
          type: "out",
          date: todayISO(),
          category: "Opening Balance Payable",
          amount: num(v.payable),
          party: "Opening Balance",
          notes: String(v.notes || "").trim() || "Opening supplier payable"
        };
        const saved = await cloudUpsert("finance", record, null);
        const mapped = mapCloudRecord("finance", saved, new Map((data.customers || []).map(c => [String(c.id), c])));
        created.push(mapped);
      ...
      if (!created.length) throw new Error("Enter an opening amount to be received or paid.");

      persist("finance", [...created, ...(data.finance || [])]);
      closeModal();
    ...
    <div className="app">
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {page === "accounts" && <Accounts jobs={data.jobs} materials={data.materials} purchases={data.purchases} finance={data.finance} onOpeningBalances={() => setModal({ type: "openingBalances", data: null })} />}
      ...
      <Drawer open={!!modal && (!sessionUser || canAccessPage(sessionUser, modal.type === "openingBalances" ? "accounts" : modal.type))} onClose={closeModal} title={modalTitle(modal, page)}>
        ...
        {modal && modal.type === "openingBalances" && <FormOpeningBalances data={data.finance} onSave={saveOpeningBalances} onCancel={closeModal} />}
